...
# Constants
#update to 100k users and 180 days
...
def player_session(env, player_id, device, country, events):
    while True:
        ...
        start_time = env.now
        events.append({
            "EventID": f"E{10000000 + len(events)}",
            "PlayerID": player_id,
        ...
        session_duration = random.randint(5 * 60, 24 * 60 * 60)  
        while env.now - start_time < session_duration:
            ...
            # Wait for next event
            # Wait between 10 seconds to 10 minutes
            # updated to 30 second to 12 hours
            yield env.timeout(random.randint(30, 12 * 60 * 60))
        ...
        # updated:  Wait between 6 hours and 180 days
        yield env.timeout(random.randint(6 * 60 * 60, 180 * 24 * 60 * 60))  
...
# update to add the time to start_date
